...
| ➕ **زيادة عندنا**  | —                                                                               | `RestaurantRate`, `SupportTicket`, `CustomerServiceEmployee`, `PushSubscription`             |
...
**عن `User.googleId` (Social Media Authentication):** عمود مش جدول — الخريطة بتطلب البند كـendpoint تحت User Auth من غير ما تحدد جدول. اللي متخزن هو **الـ`sub`** بتاع جوجل مش الإيميل، لأن جوجل بتسمح بتغيير العنوان ومطابقة بالإيميل معناها إن الحساب يتسلّم لأي حد يسجّل العنوان ده بعدين. وربط حساب موجود بيحصل **بس** لما جوجل تقول إن الإيميل متحقق — جوجل بتدّي توكن لعنوان مش متحقق منه، والربط عليه معناه إن أي حد يسجّل حساب جوجل بعنوان حد تاني يدخل حسابه. وعشان جوجل ما بتدّيش باسورد ولا تليفون، `User.password` و`Customer.phone` بقوا nullable بدل ما نخترع قيم توصل لسجل توصيل. التفاصيل في [docs/SOCIAL_AUTH.md](SOCIAL_AUTH.md).

**عن `PushSubscription`:** جدول زيادة، لكنه تنفيذ لبند رسمي مش إضافة — الخريطة بتطلب `Push Notification` كـfeature تحت `Notify Customer With Order Status` من غير ما تحدد جدول. صف لكل متصفح مش لكل عميل، لأن نفس الشخص بيشترك من موبايل ولابتوب وعايز الاتنين يرنّوا. **مفيش مزوّد ومفيش حساب**: الـVAPID زوج مفاتيح بنولّده بنفسنا وعايش في البيئة، واللي في الجدول هو نص المتصفح — نقطة النهاية اللي هو أصدرها ومفتاحين بيشفّروا الرسالة **له**، وما ينفعش يبعتوا بيهم حاجة. والقاعدة اللي بتحكم الجدول ده إن رد 404/410 من خدمة الـpush معناه إن الاشتراك خلص عمره فالصف بيتمسح، بينما فشل عادي بيسيب الصف مكانه — التفاصيل في [docs/PUSH.md](PUSH.md).
